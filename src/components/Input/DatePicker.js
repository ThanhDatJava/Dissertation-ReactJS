...
class DatePicker extends Component {
  ...
  handleBlur = (event) => {
    ...
    // Prevent the default blur event behavior
    event.preventDefault();

    // Parse the input value into a moment object
    const valueMoment = moment(value, "DD/MM/YYYY", true); // 'true' ensures strict parsing

    // Check if the parsed date is valid
    if (valueMoment.isValid()) {
      // If valid, call onChange with the parsed date (same date twice in an array)
      onChange([valueMoment.toDate(), valueMoment.toDate()]);
    } else {
      // Handle invalid date (optional, you could show an error message here)
