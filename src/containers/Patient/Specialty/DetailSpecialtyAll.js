import React, { Component } from "react";
import { connect } from "react-redux";
import "./DetailSpecialtyAll.scss";
import HomeHeader from "../../HomePage/HomeHeader";

import { getAllSpecialty } from "../../../services/userService";
import _ from "lodash";
import { LANGUAGES } from "../../../utils";

class DetailSpecialtyAll extends Component {
  constructor(props) {
    super(props);
    this.state = {
      listDetailSpecialtyAll: [],
    };
  }

  async componentDidMount() {
    let res = await getAllSpecialty();
    if (res && res.errCode === 0) {
      this.setState({
        listDetailSpecialtyAll: res.data,
      });
    }
  }

  componentDidUpdate(prevProps, prevState, snapshot) {
    if (this.props.language !== prevProps.language) {
    }
  }

  handleViewDetailSpecialty = (item) => {
    if (this.props.history) {
      this.props.history.push(`/detail-specialty/${item.id}`);
    }
  };

  render() {
    let { language } = this.props;
    let { listDetailSpecialtyAll } = this.state;
    return (
      <div className="detail-specialty-container">
        <HomeHeader />
        <div className="detail-specialty-all-body">
          {listDetailSpecialtyAll &&
            listDetailSpecialtyAll.length > 0 &&
            listDetailSpecialtyAll.map((item, index) => {
              return (
                <div className="detail-specialty-child" key={index}>
                  <div className="left">
                    <div
                      className="bg-image-specialty"
                      style={{ backgroundImage: `url(${item.image})` }}
                      onClick={() => this.handleViewDetailSpecialty(item)}
                    ></div>
                    <div className="name"> {item.name}</div>
                  </div>
                  <div className="right">
                    {item && !_.isEmpty(item) && (
                      <div
                        dangerouslySetInnerHTML={{
                          __html: item.descriptionHTML,
                        }}
                      ></div>
                    )}
                  </div>
                </div>
              );
            })}
        </div>
      </div>
    );
  }
}

const mapStateToProps = (state) => {
  return {
    language: state.app.language,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {};
};

export default connect(mapStateToProps, mapDispatchToProps)(DetailSpecialtyAll);
